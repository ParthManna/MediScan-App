...
class MainActivity8 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.learnMoreText).setOnClickListener {
            ...
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse("https://en.wikipedia.org/wiki/Psoriasis"))
    ...
    private fun updateContent(tabId: Int) {
        ...
        when(tabId) {
            R.id.overviewTab -> {
                val overView = layoutInflater.inflate(R.layout.psoriasis_overview, contentLayout, false)
                contentLayout.addView(overView)
            }
            R.id.causesTab -> {
                val causesView = layoutInflater.inflate(R.layout.psoriasis_causes, contentLayout, false)
                contentLayout.addView(causesView)
            }
            R.id.treatmentsTab -> {
                val treatmentsView = layoutInflater.inflate(R.layout.psoriasis_treatments, contentLayout, false)
                contentLayout.addView(treatmentsView)
            }
            R.id.preventionTab -> {
                val preventionView = layoutInflater.inflate(R.layout.psoriasis_prevention, contentLayout, false)
